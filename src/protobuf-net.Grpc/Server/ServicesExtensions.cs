...
namespace ProtoBuf.Grpc.Server
{
    ...
    {
        private sealed class MagicProvider<TService> : IServiceMethodProvider<TService> where TService : class
        {
            private readonly ILogger<MagicProvider<TService>> _logger;

            public MagicProvider(ILoggerFactory loggerFactory)
            {
                _logger = _logger = loggerFactory.CreateLogger<MagicProvider<TService>>();
            }
        ...
        {
            services.TryAddEnumerable(ServiceDescriptor.Singleton(typeof(IServiceMethodProvider<>), typeof(MagicProvider<>)));
        }
